...
/**
 * Class StorePredictionRequest
 * @package App\Http\Requests\Admin
 */
class StorePredictionRequest extends BasicPostRequest
{
    use RemainingJokers;

    protected $defaultMessageLangKey = 'requests.home.predictions.store.default_message';
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            if ($this->predictionsLocked()) {
                $validator->errors()->add('field', __('requests.home.predictions.store.predictions_locked'));
            }
    ...
    private function checkIfNumberOfJokersIsExceeded($validator)
    {
        ...
        if ($this->input('joker_used')) {
            $validator->errors()->add('joker_used',
                __('requests.home.predictions.store.number_of_jokers_exceeded')
            );
        }
    }
        ...
        if ($this->input('first_scorer_id')) {
            ...
            if ($this->input('joker_used')) {
                $validator->errors()->add('first_scorer_id',
                    __('requests.home.predictions.store.scorer_for_game_with_joker_exists')
                );
                return;
            }
            ...
            if ($this->input('home_team_score') == 0 && $this->input('away_team_score') == 0) {
                $validator->errors()->add('first_scorer_id',
                    __('requests.home.predictions.store.scorer_for_scoreless_game_exists')
                );
                return;
            }
            ...
            if ($this->input('home_team_score') == 0 || $this->input('away_team_score') == 0) {
                ...
                if (($this->input('home_team_score') == 0 && $game->home_team_id == $player->team_id) ||
                    ...
                    $validator->errors()->add('first_scorer_id',
                        __('requests.home.predictions.store.scorer_from_scoreless_team_exists')
                    );
                }
            }
    ...
    {
        return [
            'game_id.unique' => __('requests.home.predictions.store.duplicate_prediction')
        ];
    }
}
